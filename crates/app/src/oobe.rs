...
pub fn run(args: &[String]) -> Result<(), String> {
    ...
    println!("请在 NapCat 中配置反向 WS（每个账号一条）：");
    let token_hint = access_token.as_deref().unwrap_or("<auto-generated>");
    for account in account_ids {
        let url = build_reverse_ws_url(&napcat_base_url, &account);
        println!(
            "请配置{}的napcat实例 ws反向连接 url {} token {}",
            account, url, token_hint
        );
    }
    Ok(())
}
