...
OQQWall 的 Rust 单机版实现，采用 Functional Core / Imperative Shell 架构，用 NapCat/OneBot 接入 QQ 投稿、审核与发送 QQ 空间。当前全链路直接渲染 PNG。
...
- `OQQWALL_NAPCAT_WS_URL`：覆盖各组 `napcat_ws_url`
- `OQQWALL_NAPCAT_TOKEN`：覆盖各组 `napcat_access_token`
- `OQQWALL_PROCESS_WAITTIME_MS`：覆盖聚合窗口（毫秒）

## 配置要点
- `groups.<id>.napcat_access_token` 必填，可用 `OQQWALL_NAPCAT_TOKEN` 代替落盘。
- `mangroupid` 为审核群 ID（必填）。
- `mainqqid` 为主账号 QQ 号（必填）。
...
OSC52 复制说明：需要终端支持 OSC52。若在 tmux 内使用，建议开启剪贴板转发：
...
Konsole 需要在设置里允许 OSC52 写剪贴板（名称随版本略有差异）。
